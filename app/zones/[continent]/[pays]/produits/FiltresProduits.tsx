...
export default function FiltresProduits({ 
    ...
    // Permet de ne pas déclencher le debounce au tout premier chargement
    ...
    // La fonction centrale pour mettre l'URL à jour
    ...
    // 🎯 LE FAMEUX DEBOUNCE (Recherche instantanée avec délai)
    useEffect(() => {
        ...
        // On crée un minuteur de 400ms
        const delayDebounceFn = setTimeout(() => {
            construireUrlEtNaviguer(false); // On met à jour l'URL sans fermer le drawer
        }, 400);

        // Si l'utilisateur re-tape une lettre avant les 400ms, on annule le minuteur précédent
        return () => clearTimeout(delayDebounceFn);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [globalSearch]); // On écoute uniquement les changements sur la barre de recherche principale
    ...
    const reinitialiserFiltres = () => {
        ...
        setGlobalSearch('');
        setTypeSearch('');    // <-- NOUVEAU: on vide la recherche de types
        setRegionSearch('');  // <-- NOUVEAU: on vide la recherche de régions
    ...
    return (
        <div style={{ marginBottom: '30px' }}>
            <div style={{ display: 'flex', gap: '15px', alignItems: 'center', background: '#fff', padding: '15px', borderRadius: '12px', border: '1px solid #eaeaea', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
                <div style={{ flexGrow: 1, position: 'relative' }}>
                    <input 
                        ...
                        // Plus besoin du onKeyDown('Enter'), le debounce fait le travail tout seul !
                        style={{ width: '100%', padding: '12px 15px', borderRadius: '8px', border: '1px solid #ccc', fontSize: '16px', outline: 'none' }}
                    />
                </div>
                <button 
                    onClick={() => setIsOpen(true)}
                    style={{ background: '#222', color: '#fff', padding: '12px 24px', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '16px', display: 'flex', gap: '8px', alignItems: 'center' }}
                >
            ...
            {isOpen && (
                <div 
                    ...
                    style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', zIndex: 998, backdropFilter: 'blur(3px)' }}
                />
            )}
            ...
            <div style={{ position: 'fixed', top: 0, right: isOpen ? 0 : '-400px', width: '100%', maxWidth: '380px', height: '100vh', background: '#fff', zIndex: 999, transition: 'right 0.3s cubic-bezier(0.4, 0, 0.2, 1)', display: 'flex', flexDirection: 'column', boxShadow: '-5px 0 20px rgba(0,0,0,0.1)' }}>
                ...
                <div style={{ padding: '20px', flexGrow: 1, overflowY: 'auto' }} className="custom-scrollbar">
                    ...
                    <div style={{ marginBottom: '20px' }}>
                        <h3 style={{ margin: '0 0 15px 0', fontSize: '1.1em' }}>Région</h3>
                        <input type="text" placeholder="Chercher une région..." value={regionSearch} onChange={(e) => setRegionSearch(e.target.value)} style={{ width: '100%', padding: '10px', marginBottom: '10px', borderRadius: '6px', border: '1px solid #ddd', outline: 'none' }} />
                        <div style={{ maxHeight: '180px', overflowY: 'auto', border: '1px solid #f0f0f0', borderRadius: '6px', padding: '10px' }} className="custom-scrollbar">
                            {regionsAffichees.map(region => (
                                <label key={region} style={{ display: 'flex', alignItems: 'center', marginBottom: '12px', cursor: 'pointer' }}>
                                    <input type="checkbox" checked={selectedRegions.includes(region)} onChange={() => toggleSelection(region, selectedRegions, setSelectedRegions)} style={{ width: '18px', height: '18px', marginRight: '10px', accentColor: '#222' }} />
                                    <span style={{ fontSize: '15px' }}>{region}</span>
                                </label>
                            ))}
                            {regionsAffichees.length === 0 && <p style={{ color: '#999', fontSize: '0.9em', margin: 0 }}>Aucune région trouvée.</p>}
                        </div>
                    </div>
                ...
                <div style={{ padding: '20px', borderTop: '1px solid #eaeaea', display: 'flex', justifyContent: 'space-between', background: '#fff' }}>
                    <button onClick={reinitialiserFiltres} style={{ background: 'none', border: 'none', textDecoration: 'underline', cursor: 'pointer', fontWeight: 'bold' }}>
                        Tout effacer
                    </button>
                    <button onClick={() => construireUrlEtNaviguer(true)} style={{ background: '#222', color: '#fff', padding: '12px 24px', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' }}>
